...
- `/explore`：话题发现与关键词搜索
- `/note/:noteId`：可直接访问的笔记详情页
- `/user/:username`：公开创作者主页
- 推荐理由展示：关注作者、兴趣匹配、社区热门、新鲜内容
- 相关推荐：结合标签重合、作者关系、互动热度和内容新鲜度
- 搜索、推荐、话题统计和公开主页只处理公开笔记

### 公开与私密笔记

发布时可选择：

- **公开**：进入推荐、搜索、话题、公开主页和社区互动。
- **私密**：仅作者本人可以读取，不进入任何公共信息流。

私密笔记具备完整的数据隔离：

- `notes.visibility = private` 由 PostgreSQL RLS 限制为作者本人读取
- 图片写入非公开的 `private-note-media` Storage Bucket
- 私密图片使用一小时有效的签名 URL，不生成永久公开链接
- 私密笔记不支持点赞、收藏、评论、分享、举报和推荐行为采集
- 用户中心提供独立的“私密”内容列表
- 审核员不会通过普通前端读取私密笔记正文或图片
...
- 公开笔记支持关注作者、评论和相关推荐
- 私密详情页显示明确的“仅自己可见”状态
- 浏览、停留、点赞、收藏、评论、分享和关注行为只针对公开笔记采集
...
- 用户角色与账号状态管理
- 公开内容举报、隐藏和审核
- 用户、公开笔记、评论、访问量与独立会话统计
- 近 14 日访问趋势、热门页面和最近访问日志
- 管理员操作审计

## 推荐算法

推荐排序运行在 PostgreSQL 中，由 `get_personalized_note_ids` 返回排序后的公开笔记 ID 和推荐理由。

主要信号：

1. **兴趣匹配**：根据用户对公开笔记标签的浏览、停留和互动累计兴趣分数。
2. **社交关系**：提高已关注作者公开内容的排序权重。
3. **互动质量**：综合点赞、收藏、评论和浏览量，收藏与评论权重更高。
4. **内容新鲜度**：新内容获得时间衰减加分。
5. **作者多样性**：同一作者连续出现过多时进行降权。
6. **稳定探索**：加入确定性轻微扰动，让排序保持稳定同时保留内容探索空间。

行为权重保存在 `user_tag_preferences`，原始事件保存在 `content_events`。私密笔记不会写入推荐行为数据。
...
- `note-media`：公开笔记图片，可使用公共 URL
- `private-note-media`：私密笔记图片，仅作者可读取，使用签名 URL
- `avatars`：用户头像
...
前端推荐、评论互动、私密笔记、头像存储和用户关系所需结构均已写入 `supabase/migrations`。
